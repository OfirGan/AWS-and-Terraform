...
resource "aws_subnet" "private_subnets" {
  ...
  tags = {
    Name = "${var.vpc_name}-private-subnet-${count.index + 101}"
  }
}
...
resource "aws_eip" "internet_nat_gateway_eips" {
  count = var.availability_zones_count
  tags = {
    Name = "${var.vpc_name}-eip-internet-nat-gateway-${count.index}"
  }
...
resource "aws_nat_gateway" "internet_nat_gateways" {
  ...
  tags = {
    Name = "${var.vpc_name}-internet-nat-gateway-${count.index}"
  }
...
resource "aws_route" "route_to_internet_nat_gateway" {
  count                  = length(ws_route_table.private_route_tables[*].id)
